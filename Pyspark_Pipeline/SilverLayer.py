...
@dlt.table(
    ...
    comment = "Transformed account_transactions table"
)

def silver_account_transactions_transformed():
    df = spark.readStream.table("bronze_transactions_ingestion_cleaned")

    df = df.withColumn("channel_type",when((col("txn_channel") =="ATM") | (col("txn_channel") == "BRANCH"),lit("PHYSICAL")).otherwise("DIGITAL"))
...
@dlt.view(
    name = "silver_account_transactions_transformed_view",
    comment = "View of silver_account_transactions_transformed table"
)

def silver_account_transactions_transformed_view():
